...
class PaneCapturer:
    ...
    def collect(self, capture: Capture, since: float | None = None,
                ...
                max_stale_rounds: int = 3) -> Capture:
        """Scroll from the current position toward older history, merging frames
        into `capture` until `since` is crossed, `max_messages` is reached, or the
        top of the conversation is hit."""
        stale_rounds = 0
        while True:
            self.wait_settled()
    ...
    def scroll_until_visible(self, ts: str, max_rounds: int = 200) -> bool:
...
def capture_channel(page: Page, selectors: Selectors, channel: str | None = None,
                    ...
                    on_progress=None, log=print, settle_interval: float = 0.25) -> Capture:
    ...
    capturer.collect(capture, since=since, max_messages=max_messages, on_progress=on_progress)
...
def _capture_thread(page: Page, main: PaneCapturer, selectors: Selectors,
                    parent_ts: str, log=print, settle_interval: float = 0.25) -> list[Message]:
    if not main.scroll_until_visible(parent_ts):
    ...
    thread_capturer.collect(thread)
